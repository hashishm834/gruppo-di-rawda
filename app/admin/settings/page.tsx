...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function fetchSettings() {
      try {
        ...
        if (error && error.code !== 'PGRST116') throw error; // تجاهل خطأ عدم وجود بيانات لسه
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // 1. ترجمة النبذة
      const about_text_en = await translateText(formData.about_text, "en");
      const about_text_it = await translateText(formData.about_text, "it");
      ...
      const settingsData = {
        ...
        about_text_en: about_text_en, // الإنجليزي
        about_text_it: about_text_it, // الإيطالي
        updated_at: new Date().toISOString(),
      };

      // 3. الحفظ في الداتابيز (تحديث لو موجودة، أو إدخال لو دي أول مرة)
      if (formData.id) {
        const { error } = await supabase.schema("public").from("site_settings").update(settingsData).eq("id", formData.id);
        if (error) throw error;
      } else {
        const { error, data } = await supabase.schema("public").from("site_settings").insert([settingsData]).select().single();
        if (error) throw error;
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 border-b border-gray-800 pb-4">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">
            إعدادات الموقع (مربوطة بالمترجم 🤖)
          </h1>
          <p className="text-gray-400 mt-1">
            عدل بيانات الاتصال والنبذة لتظهر للعملاء في الموقع الرئيسي. سيتم ترجمة النبذة تلقائياً.
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-gray-800 border border-gray-700 rounded-2xl p-8 shadow-xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* نبذة عن الشركة */}
            <div className="col-span-1 md:col-span-2">
              <label className="block text-gray-300 mb-2 text-sm font-medium text-amber-400">
                نبذة عن الشركة (سيتم ترجمتها للإنجليزية والإيطالية)
              </label>
              <textarea 
                rows={5}
          ...
          <div className="mt-8 flex items-center justify-between border-t border-gray-700 pt-6">
            ...
            <button 
              type="submit" 
              disabled={isSaving}
              className="bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white px-10 py-3 rounded-lg font-bold text-lg shadow-lg hover:shadow-orange-500/25 transition-all transform hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none flex items-center gap-2"
            >
              {isSaving ? (
                <><span className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full"></span> جاري الحفظ والترجمة...</>
              ) : "حفظ الإعدادات"}
            </button>
          </div>
        </form>
