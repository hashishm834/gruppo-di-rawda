...
export default function HomePage() {
  ...
  // ✅ رسالة واتساب بالإيطالي مع تتبع
  const handleWhatsAppClick = () => {
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'conversion_event_request_quote');
    }
    const message = encodeURIComponent(
      "Salve 👋\nSono interessato ai vostri servizi di costruzione e ristrutturazione e vorrei ricevere un preventivo iniziale.\n\nSarebbe possibile essere contattato per discutere i dettagli?"
    );
    window.open(`https://wa.me/393514264494?text=${message}`, '_blank');
  };
